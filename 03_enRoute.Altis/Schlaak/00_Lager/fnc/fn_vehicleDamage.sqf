	...
	if ((_overalldamage) > 0.75) then
		{ 
			...
			[_vehicle,_case] spawn 
				...
				deletevehicle (gunner _vehicle);
				deletevehicle (commander _vehicle);
				...
				//{[_x]	spawn {sleep floor random [15,25,35];_dudesBurn = [ (_this select 0), [0,0,1], floor random 9 + 3, random [.05,.01,.015], .03 ] execvm "02_scripts\JBOY\JBOY_createFire.sqf"; sleep .01;  unassignVehicle (_this select 0); [_this select 0] allowgetin false ;}} foreach (fullCrew _vehicle);
				{unassignVehicle _x} forEach crew _vehicle;
				{moveOut _x} forEach crew _vehicle;
				
				//{moveOut _x} forEach (fullCrew _vehicle);
				...
				switch (_case) do
				{
					case 0:
					{
						...
						{ _x setdamage 1} forEach (fullCrew _vehicle);
						[_vehicle , true , true , random [130,360,620]] remoteExec ["Schlaak_fnc_fireeffects"];
					...
					case 3:
					{
						...
						sleep (floor random 60 + 60);
						{ _x setdamage 1} forEach (fullCrew _vehicle);
						[_vehicle , false , true , random [130,360,620]] remoteExec ["Schlaak_fnc_fireeffects"];
						sleep random [130,400,500];
						_vehicle setdamage 1;
					...
					case 4:
					{
						...
						[_vehicle] remoteExec ["ace_cookoff_fnc_cookOff"];
						{ _x setdamage 1} forEach (fullCrew _vehicle);
						sleep 10;
					...
					case 6:
					{
						...
						sleep (floor random 60 + 30);
						{ _x setdamage 1} forEach (fullCrew _vehicle);
						//{[_vehicle , false , true , 60] spawn Schlaak_fnc_fireeffects;}  remoteExec ["spawn", 0, false];
						[_vehicle , false , true , 60] remoteExec ["Schlaak_fnc_fireeffects"];
						sleep 40 + (random 30);
						{ _x setdamage 1} forEach (fullCrew _vehicle);
						[_vehicle , true , true , random [130,360,620]] remoteExec ["Schlaak_fnc_fireeffects"];
						sleep random [130,400,500];
						_vehicle setdamage 1;
						//{[_vehicle , true , true , random [130,360,620]] spawn Schlaak_fnc_fireeffects;}  remoteExec ["spawn", 0, false];
					...
					case 7:
					{
						...
						[_vehicle , false , true , random [15,30,40]] remoteExec ["Schlaak_fnc_fireeffects"];
						//[_vehicle] remoteExec ["ace_cookoff_fnc_engineFire"];
						sleep 40 + (random 30);
					...
					{	
						...
						//{[_vehicle] call ace_cookoff_fnc_cookOffBox;}  remoteExec ["spawn", 0, false];
						//[[_vehicle]] remoteExec ["ace_cookoff_fnc_cookOffBox"];
						sleep 15 + (random 10);
						[_vehicle , false , true , random [15,30,60]] remoteExec ["Schlaak_fnc_fireeffects"];

						//{[_vehicle] call ace_cookoff_fnc_cookOffBox;}  remoteExec ["spawn", 0, false]; //wanted to try working in mp
						//[[_vehicle]] remoteExec ["ace_cookoff_fnc_cookOffBox"];
						sleep 80 + (random 10);
						[_vehicle , true , true , random [5,10,20]] remoteExec ["Schlaak_fnc_fireeffects"];

						//[_vehicle] call ace_cookoff_fnc_cookOffBox;	//worked in sp
						//[[_vehicle]] remoteExec ["ace_cookoff_fnc_cookOffBox"];
						sleep 20 + (random 10);
				...
				if ( true ) exitwith {
						...
						if (isserver) then {
							_rndmsplat = selectRandom ["Land_Decal_ScorchMark_01_large_F"];
							sleep 20;
							...
							_mower = ["Land_ClutterCutter_large_F", getpos _vehicle, (getDir (_vehicle)) -(random 90),true] call BIS_fnc_createSimpleObject;

							_mower setpos getpos _vehicle;
							...
							(_this select 1) setDir (getDir (_this select 0)+ random 360)};
