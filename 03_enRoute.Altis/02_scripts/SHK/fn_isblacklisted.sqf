...
private ["_shape"];
_shape = _area call ShkPos_fnc_getMarkerShape;

if (_shape == "ICON") then {
  // Icon has only one position, so if it equals to the given position, then it's blacklisted.
  if ([_pos,_center] call ShkPos_fnc_isSamePosition) then {
    _return = true;
  };
...
// Markers that have an area.
  ...
  if (_shape in ["RECTANGLE","SQUARE"]) then {
    private ["_corners"];
    _corners = _area call ShkPos_fnc_getMarkerCorners;
    ...
    if (_dir % 90 != 0) then {
      ...
      for "_i" from 0 to (count _corners - 1) do {
        ...
        _posNew = [[_shiftedX,_shiftedY],_dir] call ShkPos_fnc_rotatePosition;
    ...
    _return = [_pos,_corners] call ShkPos_fnc_isInRectangle;
  } else {
    if (_shape == "CIRCLE") then {
      _return = [_pos,_area] call ShkPos_fnc_isInCircle;
    } else {
      _return = [_pos,_area] call ShkPos_fnc_isInEllipse;
    };
  };
};
