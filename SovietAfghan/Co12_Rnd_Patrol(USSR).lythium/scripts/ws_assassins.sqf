...
nul = [this OR unitname OR true,"weaponclass",chance (1-100),triggerarea (int),side OR unitname] execVM "scripts\ws_assassins.sqf";
With optional parameters:
nul = [this OR unitname OR true,"weaponclass",chance (1-100),triggerarea (int),side OR unitname,number of targets present (1-n),skill mod (0-1)] execVM "scripts\ws_assassins.sqf";


For use with ALICE (ArmA 2 only!):
Put this in the ALICE module init:
[BIS_alice_mainscope,"ALICE_civilianinit",[{nul = [_this,"",number,number,side OR unitname,number of targets present (1-n), skill (0-1)] execVM 'scripts\ws_assassins.sqf'}]] call BIS_fnc_variableSpaceAdd;
...
//
// EXAMPLES
// nul = [this,"Sa61_EP1",(40+random 35),10,west,1,1] execVM "scripts\ws_assassins.sqf";
...
//
// nul = [azim,"ran",100,10,east,2,random 0.8] execVM "scripts\ws_assassins.sqf";
...
//
// nul = [true,"LMG_Zafir_F",(20 + random 20),10,mark,1,0.5] execVM "scripts\ws_assassins.sqf";
...
//DEBUG
if (_debug) then {
player globalchat format ["scripts\ws_assassins.sqf DEBUG: _unit:%1,_target1:%2,target2:%3,_weapon:%4,chance:%5, _radius:%6, _skill: %7",_unit,_target1,_target2,_weapon,_chance,_trgsize, _skill];
};
...
if (_check) exitWith {
	...
	{[_x,_weapon,_chance,_trgsize,_target1,_target2,_skill] execVM "scripts\ws_assassins.sqf";} forEach _civarray;

	//DEBUG
	if (_debug) then {player globalchat format ["scripts\ws_assassins.sqf DEBUG: _check is %1, script will be run on _civarray:%2",_check,_civarray];};
};
...
if (_handle) exitWith {
	if (_debug) then {player globalchat format ["scripts\ws_assassins.sqf DEBUG: _unit:%1 has ws_assassin handle:%2, exiting",_unit,_handle];};
};
...
if (!(_check) && (((round(random 100))> _chance)||(_unit isKindOf "Woman")||(_unit isKindOf "Woman_EP1"))) exitWith {
	...
	if (_debug) then {player globalchat format ["scripts\ws_assassins.sqf DEBUG: exiting because random is under %1 or is woman",_chance];};
};
...
//DEBUG
if (_debug) then {player globalchat format ["scripts\ws_assassins.sqf DEBUG: ws_assassins_array:%1",ws_assassins_array];};
...
//DEBUG
if (_debug) then {
player globalchat format ["scripts\ws_assassins.sqf DEBUG: _unit:%1,_target1:%2,_target2:%3,_targettype:%4,_weapon:%5,_weaponmag:%6,_target_side:%7",_unit,_target1,_target2,_target_type,_weapon,_weaponmag,_target_side];
...
while {alive _unit} do {
	while {!(_done) && (alive _unit)} do {
		...
		{if (((side _x == _target_side) && alive _x)) then {_listclosealive set [(count _listclosealive),_x];};} foreach _listclose;
			...
			//DEBUG
			if (_debug) then {
			_string = format ["scripts\ws_assassins.sqf DEBUG: Outer Loop. _listclos: %1",_listclose];
		...
		if (((count _listclosealive) >= _target2) && ((_target1 in _listclosealive)||(typename _target1 == "SIDE"))) then {
				...
				//DEBUG
				if (_debug) then {
				_string = format ["scripts\ws_assassins.sqf DEBUG: Civ targeting _listclosealive: %2, sleeping %3",_target1, _listclosealive,_sleep];
				player globalchat _string;
				};
			doStop _unit;
			[_unit] joinSilent grpNull;
			...
			//DEBUG
			if (_debug) then {
			_string = format ["scripts\ws_assassins.sqf DEBUG: Civ engaging _target1:%1 or %2 in _listclosealive: %3, sleeping %4 seconds",_target1, (_listclosealive select (floor(random(count _listclosealive)))),_listclosealive,_sleep];
			player globalchat _string;
			};
		};
