...
*/
diag_log "start loading IRN library";
...
//adds an eventhandler which stop looting of dead bodies.
IRN_fnc_noLoot = {
    params ["_unit"];
    diag_log ["noloot for ",_unit];
    _unit addEventHandler ["InventoryOpened",
    {
        params ["_unit", "_container"];
        if (_container isKindOf "man" && !alive _container) then {
            (name _unit + " tried to loot.") remoteExec ["systemChat", 0]; 
            _looters = missionNamespace getVariable ["looters", createHashMap];
            _looters set [name _unit, (_looters getOrDefault [name _unit, 0]) + 1];
            missionNameSpace setVariable ["looters",_looters, true];
            diag_log ["looters array",_looters];
...
diag_log "finished loading IRN library";