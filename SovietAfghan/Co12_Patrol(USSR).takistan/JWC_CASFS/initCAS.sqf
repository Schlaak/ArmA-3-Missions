...
JWC_MaxD = 1000;                                // Max distance in meters allowed between requester and target
JWC_lock = true;                               // If true CAS can only be called in by the unit the addAction is tied to.
JWC_num  = selectrandom [1,1,1,1,2,2,3];                                 // Set number of CAS calls available for per unit
JWC_napalmExpire = 300;                       // Time in seconds the napalm fire will live
JWC_disallowedMunition = [""];              // Add names of disallowed munition: "JDAM", "CBU" or "NAPALM"
JWC_restrictedZones = ["nobomb1","nobomb2"];       // Add triggers as objects which defines disallowed CAS zones, like bases in TDM etc. leave empty [] if there is no restrictions.
JWC_CASarray = ["CO1","CO2"];         // Add CAS init names to this array.
