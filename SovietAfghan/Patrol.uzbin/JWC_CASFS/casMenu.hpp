class casMenu
{
	...
	controls[] = {bgEdge,bottom,top,lineTop,lineBottom,btnRequest,btnDummy,btnExit,checkEdge,checkbg,Checkbox,mapbg,map,snapText,snapTitle,help};
	...
	class Checkbox : JWC_CheckBox
	{
		...
		rows = 1;
		columns = 3;
		strings[] = {"JDAM","CBU(AP)","NAPALM"};
		checked_strings[] = {"JDAM","CBU(AP)","NAPALM"};
       	onCheckBoxesSelChanged = "if (_this select 1 == 0) then {casType = 'JDAM'}; if (_this select 1 == 1) then {casType = 'CBU'}; if (_this select 1 == 2) then {casType = 'NAPALM'}";
	};
