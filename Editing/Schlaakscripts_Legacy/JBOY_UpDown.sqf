...
_dude setVariable ["lambs_danger_disableAI",true];
...
while {alive _dude and !_done and _iterations < 12} do
{
    ...
    sleep (1 + (random 5));
    if (isNull (_dude findNearestEnemy _dude)) then
    // If no known enemies then increment iterations so that guys stop popping up and down
    {
